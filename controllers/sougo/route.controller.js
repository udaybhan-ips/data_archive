var Route = require('../../models/sougo/route');

module.exports = {
  addRoutes: async function(req, res) {
    try {
        const addRoutesRes = await Route.create(req.body);
        return res.status(200).json({
            message: 'success! Route added sucessfully',
            id: addRoutesRes
          });
    } catch (error) {
  ...
  listRoutes: async function(req, res) {
    try {
        const listRoutesRes = await Route.findAll(req.body);
        return res.status(200).json(listRoutesRes);
    } catch (error) {
        return res.status(400).json({
            message: error.message
          });
  ...
  updateRoutes: async function(req, res) {
    try {
        const listRoutesRes = await Route.updateRoute(req.body);
        return res.status(200).json(listRoutesRes);
    } catch (error) {
        return res.status(400).json({
            message: error.message
