...
module.exports = {
  ...
  getTargetCDR: async function (targetDateWithTimezone) {
    ...
    try {
      ...
      //  startTime < DATE_ADD("${targetDateWithTimezone}", INTERVAL 1 DAY)  AND 
      //  INGRPSTNTRUNKNAME = 'IPSLFIQ57APRII' AND RECORDTYPEID = 3 order by STARTTIME` ;


      const query =`SELECT 'Leafnet_006751' as type_of_service , ADDTIME(STARTTIME,'09:00:00') AS ORIGDATE, INANI, INCALLEDNUMBER,ADDTIME(DISCONNECTTIME,'09:00:00') AS STOPTIME,
          CALLDURATION*0.01 AS DURATION, SESSIONID, STARTTIME, DISCONNECTTIME, CALLDURATION, INGRESSPROTOCOLVARIANT , INGRPSTNTRUNKNAME, GW,
           ...
           startTime < DATE_ADD("${targetDateWithTimezone}", INTERVAL 1 DAY) and BILLNUM = '5050506751' AND EGRTGNAME in ('IPSSHG5423J7', 
           'IPSSHGF59EJ', 'IPSKRG5A00J', 'IPSKRG6BIIJ', 'IPSFUS10NWJ' )  AND RECORDTYPEID = 3 `;
      ...
      //     'IPSSHGF59EJ', 'IPSKRG5A00J', 'IPSKRG6BIIJ', 'IPSFUS10NWJ' )  AND RECORDTYPEID = 3 
      //     UNION
      //     SELECT 'Leafnet_0067745109' as type_of_service , ADDTIME(STARTTIME,'09:00:00') AS ORIGDATE, INANI, INCALLEDNUMBER,ADDTIME(DISCONNECTTIME,'09:00:00') AS STOPTIME, CALLDURATION*0.01 AS DURATION,
      //     SESSIONID, STARTTIME, DISCONNECTTIME, CALLDURATION, INGRESSPROTOCOLVARIANT , INGRPSTNTRUNKNAME, GW, CALLSTATUS, CALLINGNUMBER, EGCALLEDNUMBER, EGRPROTOVARIANT
      //     FROM COLLECTOR_73  
      //     where STARTTIME >= '${targetDateWithTimezone}' and startTime < DATE_ADD("${targetDateWithTimezone}", INTERVAL 1 DAY)
      //     and EGCALLEDNUMBER in ( '345650514' ) and CALLSTATUS in (16,31)
      //     and INGRPSTNTRUNKNAME in ('GSX4TOGXL3','GSX4TOGXL4','GXL3TOGSX5','GXL4TOGSX5') and RECORDTYPEID=3 `;

      // const query = `SELECT 'Leafnet_0067745109' as type_of_service , ADDTIME(STARTTIME,'09:00:00') AS ORIGDATE, INANI, INCALLEDNUMBER,ADDTIME(DISCONNECTTIME,'09:00:00') AS STOPTIME, CALLDURATION*0.01 AS DURATION,
      //      SESSIONID, STARTTIME, DISCONNECTTIME, CALLDURATION, INGRESSPROTOCOLVARIANT , INGRPSTNTRUNKNAME, GW, CALLSTATUS, CALLINGNUMBER, EGCALLEDNUMBER, EGRPROTOVARIANT
      //      FROM COLLECTOR_73  
      //      where STARTTIME >= '${targetDateWithTimezone}' and startTime < DATE_ADD("${targetDateWithTimezone}", INTERVAL 5 DAY)
      //      and EGCALLEDNUMBER in ( '345650514' ) and CALLSTATUS in (16,31)
      //      and INGRPSTNTRUNKNAME in ('GSX4TOGXL3','GSX4TOGXL4','GXL3TOGSX5','GXL4TOGSX5') and RECORDTYPEID=3 `;
