...
module.exports = {
  ...
  getTargetDate: async function (date_id) {
    try {
      ...
      if (targetDateRes.rows) {
        return {
          target_billing_month: targetDateRes.rows[0].target_billing_month,
          current_montth: targetDateRes.rows[0].current_montth,
        };
      }
      return { err: "not found" };
    } catch (error) {
  ...
  insertByBatches: async function (records, ratesData, tableName) {
    ...
    let ColumnSetValue = new pgp.helpers.ColumnSet(ColumnSetLeafnetBilling, {
      table: tableName,
    });
    try {
  ...
  genrateInvoice: async function (
    ...
    try {
      let path =
        __dirname + `\\Invoice\\Leafnet${billingYear}${billingMonth}.pdf`;

      /*** for server */

      // let path = `/app/pdf/leafnet/leafnet${billingYear}${billingMonth}.pdf`;
  ...
  sendNotification: async function (
    ...
    let mailOption = {
      ...
      to: "y_ito@ipspro.co.jp,takuya_yamada@ipspro.co.jp,m_asakura@ipspro.co.jp",
...
async function createInvoice(
  ...
  let y = generateCustomerInformation(
    customerId,
    billingYear,
    billingMonth,
    doc,
...
function tableSummary(doc, x, y, subTotal) {
  ...
  drawLine(doc, y + 48, x + 50, 500)
    ...
    .text(`${utility.numberWithCommas(subTotal)}`, x + 100, y + 20, {
      width: 100,
      align: "right",
    })
    .text(`${utility.numberWithCommas(tax)}`, x + 100, y + 35, {
      width: 100,
      align: "right",
    })
    .text(utility.numberWithCommas(totalCallAmount), x + 100, y + 50, {
      width: 100,
      align: "right",
    })
    .moveDown();
...
function generateHeader(doc, totalCallAmount) {
  doc
    ...
    .text(
      `ご請求金額合計 (Total Amount):  ${utility.numberWithCommas(
        totalCallAmount
      )}`,
      50,
      170,
...
function row(doc, heigth) {
  doc.lineJoin("miter").rect(33, heigth, 560, 40).stroke();
  return doc;
}
...
function customTable(doc, y, data, MAXY) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    textInRowFirst(
      doc,
      utility.numberWithCommas(parseInt(data[i].total_duration)),
      300,
      height,
      "right"
    );
    textInRowFirst(
      doc,
      utility.numberWithCommas(parseInt(data[i].total_amount)),
      400,
      height,
      "right"
    );
...
function generateCustomerInformation(
  ...
  drawLine(doc, y + 22)
    ...
    .text(`${customerId}-${billingYear}${billingMonth}-1`, 150, y + 25, {
      width: 100,
      align: "center",
    })
    .text(`${billingYear}/${billingMonth}/01`, 250, y + 25, {
      width: 100,
      align: "center",
    })
    .text(
      `${billingYear}/${billingMonth}/01～ ${billingYear}/${billingMonth}/${daysInMonth(
        billingMonth,
        billingYear
      )}`,
      350,
    ...
    .text(
      `${currentYear}/${currentMonthValue}/${daysInMonth(
        currentMonthValue,
        currentYear
      )}`,
      450,
  ...
  try {
    for (let i = 0; i < ratesData.length; i++) {
      if (carrierCode && companyCode) {
        if (
          ratesData[i]["carrier_code"] == carrierCode &&
          ratesData[i]["company_code"] == companyCode
        ) {
...
async function getNextInsertBatch(data, ipsRates, ratesData) {
  ...
  try {
    for (let i = 0; i < data.length; i++) {
      let obj = {};
      if (
        data[i]["term_carrier_id"] == "2GSX" ||
        data[i]["term_carrier_id"] == "AN,0"
      ) {
        data[i]["term_carrier_id"] = "5039";
      }

      const rates = await getRates(
        data[i]["billing_comp_code"],
        data[i]["term_carrier_id"],
        ratesData
      );
      ...
      const blegCallAmount =
        parseFloat(rates["rateSetup"]) +
        parseFloat(data[i]["duration_use"]) * parseFloat(rates["rateSecond"]);
      const ipsCallAmount =
        parseFloat(ipsRates["rateSetup"]) +
        parseFloat(data[i]["duration_use"]) *
          parseFloat(ipsRates["rateSecond"]);
