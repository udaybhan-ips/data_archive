...
ini_set('memory_limit', '-1');
   ...
   if(!$db) {
    ...
    where  is_deleted=false and service_type ->> 'ntt_customer' ='true' and customer_cd ='00001014' order by customer_cd  ";
...
case when call_distance is NULL then 0 else call_distance::int end, call_rate, final_call_charge, CALLCOUNT104 from (select  CDRCLASSIFICATION , CUSTOMERCODE , TERMINALTYPE , REPLACE(FREEDIALNUMBER, '-','') as free_dial ,
        ...
        '' as destination,
         ...
         from byokakin_ntt_processedcdr_$year$month where TERMINALTYPE!='その他' and customercode = '$row[1]') as foo order by  call_date, CALLTIME ";
