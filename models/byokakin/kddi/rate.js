  ...
  findAll: async function (data) {
    try {
     // console.log("in rate_KDDI"+JSON.stringify(data));
      ...
      left join (select * from byokakin_rate_approval_status where (step1_status ='pending' OR  step2_status ='pending' ) and carrier='KDDI' ) as rj
  ...
  updateRates: async function (data) {
    ...
    try {
      ...
      const resUpdate = await db.queryByokakin(queryUpdate, []);
        ...
        await checkApprovalStatus(data, 'KDDI');
