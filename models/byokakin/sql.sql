...
create table byokakin_ntt_Koteihi_202305 (cdrid serial, did varchar(30), carrier varchar, service_name varchar, amount numeric(16,0),
 TAXCLASSIFICATION varchar, DAILYDISPLAY varchar, date_added timestamp without time zone, carrier_name varchar);

create table byokakin_ntt_rawcdr_inbound_202305 (cdrid serial, customername varchar, did varchar(30), calldate timestamp without time zone,
...
create table byokakin_ntt_rawcdr_outbound_202305 (cdrid serial, customername varchar, parentdid varchar(30), calltype varchar,
...
create table byokakin_ntt_processedcdr_202305 (cdrid bigint, cdrclassification varchar(10), customercode varchar(10), terminaltype varchar(10),
...
select distinct(did) from byokakin_kddi_infinidata_202305 where did not  in ( select distinct(freedialnumber) from byokakin_kddi_processedcdr_202305 where cdrclassification='INBOUND' ) ;
select distinct(did) from byokakin_kddi_raw_cdr_202305 where did not  in ( select distinct(freedialnumber) from byokakin_kddi_processedcdr_202305 where cdrclassification='OUTBOUND' ) ;
select distinct(did) from byokakin_ntt_rawcdr_inbound_202305 where did not  in  ( select distinct( callingnumber)   from byokakin_ntt_processedcdr_202305 where cdrclassification='INBOUND' )
select distinct(did) from byokakin_ntt_rawcdr_outbound_202305  where callcharge>0 and  did not  in  ( select distinct( freedialnumber)   from byokakin_ntt_processedcdr_202305 where cdrclassification='OUTBOUND' );

select total_a, free, total_b, did, total_a-total_b from ( select count(*) as total_a,  ((regexp_replace(freedialnumber, '[^0-9]', '', 'g'))) as
 free  from byokakin_kddi_processedcdr_202305 where cdrclassification='OUTBOUND' group by regexp_replace(freedialnumber, '[^0-9]', '', 'g')) as 
 lj join ( select count(*) as total_b,  ((regexp_replace(did, '[^0-9]', '', 'g'))) as did  from byokakin_kddi_raw_cdr_202305 group by 
 regexp_replace(did, '[^0-9]', '', 'g')) as rj on (lj.free=rj.did);

 copy (select customercode, cdr_cost_subtotal, round(cdr_cost_subtotal+(cdr_cost_subtotal*.1)) as cdr_cost_subtotal_with_tax from byokakin_billing_history where cdrmonth='2023-03-1' and carrier ='KDDI') to '/tmp/kddi_202305.csv'  WITH (FORMAT CSV, HEADER);  ;