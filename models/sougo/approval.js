...
module.exports = {
    ...
    addApprovalStatus: async function(data) {
        try {
            ...
            valueArray.push((data.company_code));
            valueArray.push(data.company_name);
    ...
    sendApprovalNotification: async function(reqData){
        let subject = `Approval Notification for ${reqData.company_name} of ${utility.dateVsMonthsWithoutZero[reqData.billing_month]}`;
        let html = `<div>
            <div> Hi Team, </div>
            <div> Below is the billing status of ${reqData.company_name} Sougo. This is approved by ${reqData.approved_by}.</div>
            <div> Thank you </div>
        </div>`;
