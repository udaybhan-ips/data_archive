...
module.exports = {
  ...
  createTable: async function (tableName, databaseName = "cdr_sonus") {
    try {
  ...
  getTargetInboundCDR: async function (targetDateWithTimezone) {
  ...
  getTargetOutboundCDR: async function (targetDateWithTimezone) {
    ...
    try {
      const query = `SELECT ADDTIME(STARTTIME,'09:00:00') AS ORIGDATE, INANI, INCALLEDNUMBER,ADDTIME(DISCONNECTTIME,'09:00:00') AS STOPTIME,
       CALLDURATION*0.01 AS DURATION, SESSIONID, STARTTIME, DISCONNECTTIME, CALLDURATION, INGRESSPROTOCOLVARIANT , INGRPSTNTRUNKNAME, GW,
        ...
        where STARTTIME >= '${targetDateWithTimezone}' and startTime < DATE_ADD("${targetDateWithTimezone}", INTERVAL 1 DAY)   
        and  RECORDTYPEID = 3 and CALLSTATUS in (16,31) and gw in ('nfpgsx4','IPSGSX5') 
        and EGRPROTOVARIANT like '%0xfb,6015%' order by STARTTIME `
  ...
  insertByBatches: async function (tableName, records, __type) {
    ...
    for (let i = 0; i < chunkArray.length; i++) {
      const data = getNextInsertBatch(chunkArray[i], __type);
      res = await db.queryBatchInsertSonusWithColumnSet(data, ColumnSetCOMSQ, { table: tableName });
      resArr.push(res);
    }
...
function getNextInsertBatch(data, __type) {
  ...
  try {
    for (let i = 0; i < data.length; i++) {
      ...
      obj['in_outbound'] = 0;
      obj['dom_int_call'] = 0;
      obj['orig_carrier_id'] = __type == 'INBOUND' ? getOrigCarrierID(data[i]['INGRESSPROTOCOLVARIANT']) : getOrigCarrierID(data[i]['EGRPROTOVARIANT']);
      obj['term_carrier_id'] = __type == 'INBOUND' ? getTermCarrierID(data[i]['INGRESSPROTOCOLVARIANT']) : getTermCarrierID(data[i]['EGRPROTOVARIANT']);
      obj['transit_carrier_id'] = __type == 'INBOUND' ? getTransistCarrierID(data[i]['INGRESSPROTOCOLVARIANT']) : getTransistCarrierID(data[i]['EGRPROTOVARIANT']);;
      obj['selected_carrier_id'] = __type == 'INBOUND' ? getSelectedCarrierID(data[i]['INGRESSPROTOCOLVARIANT']) : getSelectedCarrierID(data[i]['EGRPROTOVARIANT']);
      obj['billing_company_code'] = "COMSQ";
      obj['trunk_port'] = 0;
