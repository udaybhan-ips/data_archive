      ...
      try {
          const query=`select bill_no, customer_code,(select customer_name from m_customer where customer_cd=customer_code limit 1)
           ...
           (select status  from kickback_sougo_approval_history where  
            invoice_number::int=kickback_history.bill_no order by approved_date desc limit 1) as status,
             (select approved_date  from kickback_sougo_approval_history where 
             invoice_number::int=kickback_history.bill_no order by approved_date desc limit 1) as approved_date,
              (select approved_by  from kickback_sougo_approval_history where 
              invoice_number::int=kickback_history.bill_no order by approved_date desc limit 1) as approved_by,
