...
createSummaryData: async function(customer_id, year, month) {
  ...
  try {
      ...
      const getSummaryData=`select SUM(FLOOR(total_amount))as total_amount, sum(total_duration) as total_duration  from  (select b.Term_Carrier_ID  || '-' || c.Carrier_Name as carrier_name_id, SUM(b.Duration_Use) as total_duration, round(SUM(a.total_amount), 2) as total_amount from CDR_SONUS_BILLING a, CDR_SONUS b, CDR_SONUS_RATE c where a.CDR_ID = b.CDR_ID and a.Rate_ID = c.Rate_ID and to_char(b.start_time, 'MM-YYYY') = '${month}-${year}' group by b.Term_Carrier_ID, c.Carrier_Name )as foo` ;
