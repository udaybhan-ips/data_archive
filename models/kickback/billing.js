...
module.exports = {
  ...
  getKickCompCallsInfo: async function (customer_id) {
    ...
    try {
      const query = `select count(*) as total, sum(duration)/60 as total_duration ,kick_company from billcdr_202303 
  ...
  getTargetDateByTermUse: async function (customerInfo) {
    ...
    try {
      ...
      OVER (order by start_time asc) as cum_sum from billcdr_202303 where kick_company='${customerInfo.customer_id}' and term_use=2 ) as t
  ...
  getTargetCDR: async function (kickCompany, service_type, year, month, _03_numbers_arr, isExceed, exceedLimitTime) {
    ...
    try {
      ...
      from billcdr_202303 where term_ani in (${_03_numbers}) and duration>1 and call_status in (16, 31) group by term_ani order by term_ani  `;
      } else {
        if (isExceed || kickCompany == '00000697') {
          ...
          from billcdr_202303 where duration>1 and call_status in (16, 31) and term_ani in (select substring(_03_numbers, 2, 10) as _03_numbers 
  ...
  getTargetCDRByTermUse: async function (kickCompany, year, month, _03_numbers_arr, targetDateTime, term_use) {
    ...
    try {
      ...
      const query = `select  * from  billcdr_202303 ${where} `;
      console.log("query==" + query);
      const data = await db.queryIBS(query);
